...
import UserAndRoleMappingForm from './UserAndRoleMappingForm';
...
const createData = (UserName, RoleName) => {
  return { UserName, RoleName };
};

function UserAndRoleMappingTable() {
  const baseURL = 'https://staffcentral.azurewebsites.net/api';
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(5);
  const [order, setOrder] = useState('asc');
  const [orderBy, setOrderBy] = useState('empFName');
  ...
  const refreshList = () => {
    axios.get(baseURL + '/RoleMapping/GetAllUserRoleMapingList')
      .then(response => {
  ...
  useEffect(() => {
    setFilteredRows(
      data.filter(
        (row) =>
          row.empFName &&
          row.empFName.toLowerCase().includes(search.toLowerCase())
      )
    );
  ...
  const handleSearchChange = (event) => {
    ...
    const filteredData = data.filter((row) => 
      row.empFName.toLowerCase().includes(searchValue) || 
      row.roleTexts.toLowerCase().includes(searchValue)
    );
  ...
  const [empcode, setSelectedRecordId] = useState(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(baseURL + '/RoleMapping/DeleteUserRoleMapping?EmpCode=' + empcode, {
        method: 'POST',
        headers: {
  ...
  const downloadExcel = () => {
    const filteredData = filteredRows.map(row => ({ 'User Name': row.empFName, 'Roles': row.roleTexts }));
    const ws = XLSX.utils.json_to_sheet(filteredData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "User_Roles");
    XLSX.writeFile(wb, "User_Roles.xlsx");
  };

  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.autoTable({
      head: [['User Name', 'Roles']],
      body: filteredRows.map(row => [row.empFName, row.roleTexts]),
    });
  ...
  return (
    <Paper sx={{ width: '100%', overflow: 'hidden', mb: 2 }} className='table-card'>
      ...
      {showDiv && (
        <Box>
          {showDiv ? <UserAndRoleMappingForm refreshList={refreshList} dataHide={setShowDiv} roleData={tableData} /> : null}
        </Box>
      )}
      ...
      <Grid container spacing={2} className='table-top-head'>
        <Grid item xs={2} sx={{ mt: 0 }} className='file-upload'>
          ...
          <Item>
            <CSVLink data={handleCsvDownload()} filename={'user_Roles.csv'}>
        ...
        <Grid item xs={2} textAlign={'right'}>
          ...
          <Button variant="contained" className='primary' onClick={handleToggle}>
          ...
          <TableHead>
            ...
            <TableCell style={{ width: 50 }}>S.No</TableCell>
              <TableCell>
                ...
                <TableSortLabel
                  active={orderBy === 'empFName'}
                  direction={orderBy === 'empFName' ? order : 'asc'}
                  onClick={() => handleRequestSort('empFName')}
                >
                  User Name
                </TableSortLabel>
              </TableCell>
...
export default UserAndRoleMappingTable;
